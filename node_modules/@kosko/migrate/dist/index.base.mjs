...
import { loadString, getResourceModule } from '@kosko/yaml';
...
let DEFAULT_FORMAT = MigrateFormat.ESM;
function jsonStringify(data) {
    return stringify(data, void 0, "  ");
}
async function generateComponent(manifest) {
    let { apiVersion , kind , ...data } = manifest, name = camelcase(kind), mod = await getResourceModule(manifest);
    return mod ? {
        name,
...
async function migrate(data, options = {}) {
    ...
    for (let { path , names  } of (format === MigrateFormat.CJS && (output += `"use strict";\n\n`), function(components) {
        let importMap = {};
