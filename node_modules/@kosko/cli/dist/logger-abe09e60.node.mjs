...
import { exit, env, stderr, stdout, execPath, execArgv } from 'node:process';
...
async function getESMEntry(cwd) {
    ...
    let path = join(pkgPath, "..", pkg.module);
    return pathToFileURL(path).toString();
}
async function importEnvNode(cwd) {
    let envPath = resolveFrom(cwd, "@kosko/env"), envs = [];
    if (envs.push(require(envPath)), "1" !== env.ESM_IMPORT_DISABLED) {
        let envModUrl = await getESMEntry(dirname(envPath));
...
async function handler(options) {
    ...
    if (await setupEnv(config, args), config.require.length) {
        let req = createRequire(join(args.cwd, "noop.js"));
...
async function runWithLoaders(options) {
    try {
        ...
        if (err instanceof SpawnError) throw new CLIError(err.message, {
            output: "",
            code: err.exitCode
        });
        throw err;
...
//# sourceMappingURL=logger-abe09e60.node.mjs.map
