"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.addSchema = void 0;
const validate_1 = require("@kubernetes-models/validate");
const IoK8sApimachineryPkgApisMetaV1ObjectMeta_1 = require("@kubernetes-models/apimachinery/_schemas/IoK8sApimachineryPkgApisMetaV1ObjectMeta");
const IoK8sApiCoordinationV1LeaseSpec_1 = require("./IoK8sApiCoordinationV1LeaseSpec.js");
const schema = {
    "properties": {
        "apiVersion": {
            "type": "string",
            "enum": [
                "coordination.k8s.io/v1"
            ]
        },
        "kind": {
            "type": "string",
            "enum": [
                "Lease"
            ]
        },
        "metadata": {
            "oneOf": [
                {
                    "$ref": "io.k8s.apimachinery.pkg.apis.meta.v1.ObjectMeta#"
                },
                {
                    "type": "null"
                }
            ]
        },
        "spec": {
            "oneOf": [
                {
                    "$ref": "io.k8s.api.coordination.v1.LeaseSpec#"
                },
                {
                    "type": "null"
                }
            ]
        }
    },
    "type": "object",
    "required": [
        "apiVersion",
        "kind"
    ]
};
function addSchema() {
    (0, IoK8sApimachineryPkgApisMetaV1ObjectMeta_1.addSchema)();
    (0, IoK8sApiCoordinationV1LeaseSpec_1.addSchema)();
    (0, validate_1.register)("io.k8s.api.coordination.v1.Lease", schema);
}
exports.addSchema = addSchema;
