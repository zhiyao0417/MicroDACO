import { register } from "@kubernetes-models/validate";
import { addSchema as IoK8sKubeAggregatorPkgApisApiregistrationV1beta1APIServiceCondition } from "./IoK8sKubeAggregatorPkgApisApiregistrationV1beta1APIServiceCondition.mjs";
const schema = {
    "properties": {
        "conditions": {
            "items": {
                "$ref": "io.k8s.kube-aggregator.pkg.apis.apiregistration.v1beta1.APIServiceCondition#"
            },
            "type": "array",
            "nullable": true
        }
    },
    "type": "object"
};
export function addSchema() {
    IoK8sKubeAggregatorPkgApisApiregistrationV1beta1APIServiceCondition();
    register("io.k8s.kube-aggregator.pkg.apis.apiregistration.v1beta1.APIServiceStatus", schema);
}
